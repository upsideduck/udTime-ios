...
//  udTime
//
//  Created by Johan Adell on 30/07/14.
...
- (void) viewWillAppear:(BOOL)animated{
    ...
    NSLog(@"On BreakAdd %@",self.managedObjectContext);
...
-(void)setupStartValues{
    ...
    self.starttimePicker.date = self.lowerDateLimit;
    self.starttimePicker.date = self.upperDateLimit;
    self.endtimePicker.minimumDate = self.lowerDateLimit;
    self.endtimePicker.maximumDate = self.upperDateLimit;
    self.endtimePicker.date = self.upperDateLimit;
    self.endtimePicker.date = self.upperDateLimit;
    
    self.starttimeCell.detailTextLabel.text = [dateTimeformatter stringFromDate:self.lowerDateLimit];
    self.endtimeCell.detailTextLabel.text = [dateTimeformatter stringFromDate:self.upperDateLimit];
...
- (IBAction)saveBreak:(UIBarButtonItem *)sender{
    ...
    NSDictionary *parameters = @{@"action": @"setbreak",
      ...
      parameters:parameters
         success:^(AFHTTPRequestOperation *operation, id responseObject) {
             ...
             if(![udTimeServer successOnResult:responseObject onAction:@"results.login"]) {
                 ...
                 return;
             }else if(![udTimeServer successOnResult:responseObject onAction:@"results.setbreak"]) {
                 NSLog(@"Update error");
                 [udTimeServer showServerMessage:@"Could not add break"];
                 return;
             }
